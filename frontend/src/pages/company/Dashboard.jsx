    ...
    api.get("/auth/notifications")
      .then((res) => {
        ...
        const filtered = all.filter((n) =>
          ["candidate_shortlisted", "interview_scheduled", "assessment_sent"].includes(n.status || n.type)
        );
      ...
      <div className="bg-[var(--bg-secondary)] rounded-lg shadow-sm border border-[var(--border-color)]">
        <div className="p-4 border-b border-[var(--border-color)]">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1">
            Shortlisted candidates and interview workflow alerts.
          </p>
        </div>
        {notifications.length === 0 ? (
          <p className="p-4 text-sm text-[var(--text-secondary)]">No shortlist notifications yet.</p>
        ) : (
          <div className="max-h-80 overflow-y-auto divide-y divide-[var(--border-color)] relative">
            {notifications.map((n) => (
              <div key={n.id} className="p-4 relative group">
                <button 
                ...
                <p className="text-sm text-[var(--text-primary)] font-medium pr-6">{n.message}</p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">
